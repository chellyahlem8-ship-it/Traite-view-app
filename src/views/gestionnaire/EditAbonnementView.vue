    ...
    <main class="page-content">
      ...
      <Transition name="slide-down">
        <div v-if="success" class="toast toast--success" role="status">
          ...
          <span>Abonnement modifié avec succès !</span>
        </div>
      </Transition>
      ...
      <div v-else class="abonnement-form-card">
        ...
        <div class="form-header">
          ...
          <div>
            <h2 class="form-title">Modifier l'abonnement</h2>
            <p class="form-subtitle">Mettez à jour les informations de l'abonnement.</p>
          </div>
        </div>
        ...
        <form class="form-body" novalidate @submit.prevent="handleSubmit">
          ...
          <section class="form-section">
            ...
            <div class="field-group">
              <label class="field-label">
                Durée <span class="required-star">*</span>
              </label>
              <div class="input-wrapper">
                <svg class="input-icon" viewBox="0 0 20 20" fill="currentColor">
                  <path fill-rule="evenodd"
          ...
          <section class="form-section">
            ...
            <div class="field-row">
              ...
              <div class="field-group">
                ...
                <span v-if="dureeCalculee" class="field-hint">
                  ⏱ {{ dureeCalculee }}
                </span>
              </div>
...
// ================================================================
// EditAbonnementView.vue
// Modification d'un abonnement existant — même pattern que EditTier
// ================================================================
...
// ── Id depuis l'URL (/abonnements/:id/edit) ───────────────────
const abonId = computed<number>(() => Number(route.params.id))

// ── Composable formulaire ─────────────────────────────────────
const {
  loading, success, generalError, errors,
  fetchOne, fetchSocietesEtTarifs, updateAbonnement,
} = useAbonnementForm()
...
const loadingInit = ref<boolean>(false)
...
onMounted(async () => {
  ...
  try {
    // Charger les listes + l'abonnement en parallèle
    const [{ societes: s, tarifs: t }, ab] = await Promise.all([
      fetchSocietesEtTarifs(),
      fetchOne(abonId.value),
    ])
    ...
    generalError.value = e instanceof Error
      ? e.message
      : 'Impossible de charger les données de l\'abonnement.'
  } finally {
...
async function handleSubmit(): Promise<void> {
  const ok = await updateAbonnement(abonId.value, form)
  if (ok) {
    // Laisser le toast de succès visible 1,5 s puis rediriger
    setTimeout(() => { router.push({ name: 'Abonnements' }) }, 1500)
  }
}

function goBack(): void {
  router.push({ name: 'Abonnements' })
}
</script>
...
.btn {
  ...
  &--primary {
    ...
    &:hover:not(:disabled) { box-shadow: 0 4px 16px rgba(91, 33, 182, 0.36); transform: translateY(-1px); }
