<template>
  <div class="page-layout">
    ...
    <main class="page-content">
      ...
      <!-- Loader initial -->
      <div v-if="loadingTier" class="loader-wrapper">
      ...
      <template v-else>
        ...
        <div class="breadcrumb">
          <button class="breadcrumb-link" @click="router.push({ name: 'Tiers' })">← Retour aux tiers</button>
        </div>
          ...
          <div class="cb-panel-header">
            ...
            <div>
              ...
              <p class="cb-panel-subtitle">
                Pour le tiers : <strong>{{ tierName }}</strong>
              </p>
            </div>
          </div>
          ...
          <div class="cb-form">
            ...
            <div class="field-group">
              ...
              <template v-else>
                ...
                <span class="field-hint">{{ ribDigitCount }}/20 chiffres</span>
              </template>
            </div>
            ...
            <div class="field-group">
              ...
              <div class="field-fixed">
                ...
                <span class="fixed-name">{{ tierName }}</span>
              </div>
            ...
            <div class="cb-form-actions">
              ...
              <button class="btn btn--secondary" @click="router.push({ name: 'Tiers' })">
                Annuler
              </button>
            </div>
          </div>
...
onMounted(async () => {
  ...
  try {
    const [tierRes, banquesRes] = await Promise.all([
      tiersApi.getOne(tierId),
      banquesApi.getAll(),
    ])
    tierName.value = tierRes.data.raison_sociale
...
.page-content {
  ...
  padding: 40px 24px; gap: 16px; overflow-y: auto;
}
...
.modal-close {
  ...
  font-size: 16px; color: #9ca3af; padding: 4px 8px; border-radius: 8px;
  line-height: 1;
}
...
.field-input {
  ...
  font-size: 14px; font-family: 'Outfit', sans-serif; color: #1f2937;
  background: #fff; transition: border-color 0.2s; outline: none; width: 100%;
}
...
.field-fixed {
  display: flex; align-items: center; gap: 10px;
  padding: 10px 14px; border: 1.5px solid #e9e5f5;
  border-radius: 10px; background: #faf9ff;
}
.fixed-badge {
  font-size: 11px; font-weight: 700; padding: 3px 8px;
  border-radius: 20px; font-family: 'Outfit', sans-serif;
...
.btn {
  ...
  gap: 6px; padding: 10px 22px; border-radius: 10px;
  font-size: 14px; font-weight: 600; cursor: pointer;
  border: none; transition: all 0.2s; font-family: 'Outfit', sans-serif;
}
.btn--primary {
  background: linear-gradient(135deg, #7c3aed, #6d28d9);
  color: #fff; box-shadow: 0 3px 10px rgba(109,40,217,0.3);
}
