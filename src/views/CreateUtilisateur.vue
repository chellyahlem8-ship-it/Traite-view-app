<template>
  <Sidebar />
  <div class="create-utilisateur-page">
...
const {
  formState, errors: rawErrors, loading, success,
  generalError, societes, createUtilisateur, resetForm,
} = useUtilisateurs()
...
watch(success, (val) => { if (val) setTimeout(() => resetForm(), 3000) })

async function handleSubmit() { await createUtilisateur() }
function handleCancel() { router.back() }
</script>

<style scoped>
.create-utilisateur-page {
  min-height: 100vh;
  ...
  @media (max-width: 768px) { padding: 80px 16px 32px; }
}
...
@media (max-width: 600px) {
  .create-utilisateur-page { padding: 0; background: #fff; justify-content: flex-start; }
