...
<Sidebar />
  <div class="create-compte-page">
    ...
    <FormCreerCompteBancaire
      v-model="formData"
      :loading="loading"
      :errors="errors"
      :banques="banques"
      :loading-banques="loadingBanques"
      :titulaires="titulaires"
      :loading-titulaires="loadingTitulaires"
      @submit="handleSubmit"
      @cancel="handleCancel"
    />
...
import type {
  ...
  CompteBancaireFormErrors,
} from '@/components/FormCreerCompteBancaire.vue'
import { useComptesBancaires } from '@/composables/useComptesBancaires'
...
const formData = computed<CompteBancaireFormData>({
  get: () => ({
    rib:            formState.rib,
    adresse_agence: formState.adresse_agence,
    banque_id:      formState.banque_id,
    titulaire_type: formState.titulaire_type,
    titulaire_id:   formState.titulaire_id,
  }),
  set: (val: CompteBancaireFormData) => {
    formState.rib            = val.rib
    formState.adresse_agence = val.adresse_agence
    formState.banque_id      = val.banque_id
    formState.titulaire_type = val.titulaire_type
    formState.titulaire_id   = val.titulaire_id
  },
})

const errors = computed<CompteBancaireFormErrors>(() => rawErrors)
...
@media (max-width: 600px) {
  .create-compte-page { padding: 0; background: #fff; justify-content: flex-start; }
