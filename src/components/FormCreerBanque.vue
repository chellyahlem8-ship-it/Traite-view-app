  ...
  <div class="banque-form-card">
    ...
    <div class="form-header">
      <div class="header-icon">
        <!-- Icône "banque / institution" -->
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
          <path stroke-linecap="round" stroke-linejoin="round"
            d="M3 21h18M3 10h18M5 6l7-3 7 3M4 10v11M20 10v11M8 10v11M12
               10v11M16 10v11" />
        </svg>
      </div>
      <div>
        <h2 class="form-title">Nouvelle Banque</h2>
        <p class="form-subtitle">Enregistrez un établissement bancaire.</p>
      </div>
    </div>
      ...
      <section class="form-section">
        ...
        <div class="field-group">
          <label class="field-label">
            Nom de la Banque <span class="required-star">*</span>
          </label>
          <div class="input-wrapper">
            ...
            <input
              :value="modelValue.nomBanque"
              type="text"
              class="field-input field-input--icon"
              :class="{ 'field-input--error': errors?.nomBanque }"
              placeholder="Ex : Banque Nationale de Tunisie"
              @input="patch('nomBanque', ($event.target as HTMLInputElement).value)"
            />
          </div>
          <span v-if="errors?.nomBanque" class="field-error-text">
            {{ errors.nomBanque }}
          </span>
        </div>
      </section>
      ...
      <div class="form-actions">
        <button type="submit" class="btn btn--primary" :disabled="loading">
          <span class="btn-inner">
            ...
            {{ loading ? 'Enregistrement…' : 'Créer la Banque' }}
          </span>
        </button>
...
// ─────────────────────────────────────────────────────────────────────
//  FormCreerBanque.vue  –  Composant 100 % présentationnel
//  ➜ Aucune logique API, aucun store, aucun onMounted
//  ➜ Toute la logique métier est gérée par le parent (CreateBanque.vue)
// ─────────────────────────────────────────────────────────────────────

/** Structure plate des données du formulaire */
export interface BanqueFormData {
  nomBanque: string
}

/** Erreurs de validation par champ, fournies par le parent */
export interface BanqueFormErrors {
  nomBanque?: string
}
...
const props = withDefaults(
  defineProps<{
    /** Données liées via v-model */
    modelValue: BanqueFormData
    /** true pendant la soumission (désactive le bouton) */
    loading?: boolean
    /** Erreurs de validation par champ */
    errors?: BanqueFormErrors
  }>(),
  {
    loading: false,
    errors: () => ({}),
...
const emit = defineEmits<{
  (e: 'update:modelValue', value: BanqueFormData): void
  (e: 'submit'): void
  (e: 'cancel'): void
}>()

// ── Helpers ────────────────────────────────────────────────────────────
/** Met à jour un seul champ sans muter la prop directement */
function patch<K extends keyof BanqueFormData>(key: K, value: BanqueFormData[K]) {
